...
# Load the Excel file
def prepare_data_knn(file_path):
...
def find_nearest_players_knn(model, scaler, player_names, X, need, top_n=3):
    need_scaled = scaler.transform([need])
    distances, indices = model.kneighbors(need_scaled, n_neighbors=top_n)
    
    nearest_players = player_names.iloc[indices[0]].tolist()
...
if __name__ == "__main__":
    ...
    nearest_players = find_nearest_players_knn(model, scaler, player_names, X, need, top_n=3)
